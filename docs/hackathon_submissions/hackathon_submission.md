...
EcoAgent MCP Server is a fully MCP (Model Context Protocol)-compliant server that exposes powerful sustainability tools for AI agents. Built on top of the EcoAgent sustainability assistant, it allows AI agents to access carbon footprint calculations, personalized environmental recommendations, and real-time environmental data through the standardized MCP protocol.
...
The EcoAgent MCP Server provides MCP-compatible AI agents with powerful sustainability tools to help users make environmentally conscious decisions. All tools are designed for individual consumers and fully comply with the Model Context Protocol specification.